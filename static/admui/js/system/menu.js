(function () {
    ...
    window.Content = App.extend({
        ...
        updateOrder: function ($item) {
            ...
            $.ajax({
                url: $.ctx + 'right/update_top_order',
                type: 'POST',
        ...
        saveData: function () {
            ...
            $.ajax({
                url: $.ctx + 'right/save',
                type: 'POST',
                data: {menu: JSON.stringify(this.menuObj), type:'sub'},
                dataType: 'JSON',
                success: function (data) {
        ...
        setMenu: function (data, ID) {
            ...
            if (ID !== "") {
                ...
                if (JSON.parse($navMenu.data('children')) === null||$navMenu.data('children').length==0) {
                    this.isShow('hide');
                } else {
                    this.isShow('show');
                }
            ...
            $('#controlMenu').formValidation($.po('formValidation', {
                ...
                .on('success.form.fv', function (e) {
                    ...
                    $.ajax({
                        url: $.ctx + 'right/save',
                        type: 'POST',
                        data: {menu: JSON.stringify(menuData), type:'top'},
                        dataType: 'JSON',
                        success: function (data) {
                            if (data.success) {
